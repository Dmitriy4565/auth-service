...
// GetValidTwoFactorCode находит действующий код двухфакторной аутентификации
func (r *UserRepository) GetValidTwoFactorCode(userID uint, code string) (*models.TwoFactorCode, error) {
    var tfCode models.TwoFactorCode
    err := r.db.Where("user_id = ? AND code = ? AND used = false AND expires_at > ?", 
        userID, code, time.Now()).First(&tfCode).Error
    if err != nil {
        return nil, err
    }
    return &tfCode, nil
